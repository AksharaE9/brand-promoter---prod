...
/**
 * offerLetterUpload.js
 * Route: POST /api/candidates/bulk-upload/offer-letter
 *        GET  /api/candidates/bulk-upload/offer-letter/template/download
 *        GET  /api/candidates/bulk-upload/offer-letter/:jobId
 *        GET  /api/candidates/bulk-upload/offer-letter/:jobId/report
 */
...
const { processOfferLetterUpload, getJobStatus } = require('../jobs/bulkOfferLetterUpload.processor');
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, `bulk_offer_${uuidv4()}${ext}`);
  },
});
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const filename = `offer_letter_bulk_upload_template.${format}`;
    ...
    const schema = [
      ...
      { key: 'offerDecision', label: 'Offer Decision',  required: false },
    ...
    if (format === 'xlsx') {
      try {
        ...
        console.error('Offer letter template download failed:', err);
    ...
    const sampleRow = ['Jane Smith', '+919876543210', 'jane@example.com', 'Product Manager', '2024-06-15', 'Accepted', 'IIM Ahmedabad', 'Ahmedabad', 'MBA', 'Referral', 'Akshara Enterprises'];
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    setImmediate(async () => {
      try {
        await processOfferLetterUpload({
          jobId,
        ...
        console.error(`[OfferLetterUploadRoute] Job ${jobId} error:`, err.message);
      }
    });
    ...
    res.status(202).json({
      ...
      data: {
        jobId,
        status: 'active',
        message: 'Offer letter candidates file accepted. Job queued for background processing.',
      },
    });
  })
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="bulk_offer_report_${jobId}.${ext}"`);
