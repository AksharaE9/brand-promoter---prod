...
export default function BulkInterviewUploadModal({ isOpen, onClose, onSuccess }) {
  ...
  const downloadTemplate = async () => {
    try {
      setErrorMsg(null);
      await downloadAuthenticatedFile('/interviews/bulk-upload/template/download?format=xlsx', 'interview_schedule_bulk_template.xlsx');
    } catch (err) {
  ...
  const handleUpload = async () => {
    ...
    if (defaultMode) {
      formData.append('defaultMode', defaultMode);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4">
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <div>
            <h3 className="text-base font-bold text-slate-900">Bulk Interview Schedule Upload</h3>
            <p className="text-xs text-slate-500">Schedule multiple interview rounds in bulk via spreadsheet</p>
          </div>
          <button
            onClick={onClose}
            disabled={uploading}
          ...
          {!jobStatus ? (
            <>
              <div className="flex items-center justify-between">
                <label className="text-xs font-semibold text-slate-700">Import Options</label>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold text-slate-500 block">Default Round</label>
                  <select
                    value={defaultRound}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold text-slate-500 block">Default Meeting Mode</label>
                  <select
                    value={defaultMode}
                  ...
                  {file ? (
                    ...
                    <div>
                      <p className="text-xs font-semibold text-slate-700">
                        Drag and drop your schedules CSV/XLSX file here
                      </p>
                      <p className="text-[11px] text-slate-400 mt-1">Supports CSV, XLSX, XLS up to 10MB (max 500 rows per upload)</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="grid grid-cols-4 gap-2 pt-2 text-center">
                ...
                <div className="p-2 bg-emerald-50 rounded-xl border border-emerald-100">
                  <p className="text-[10px] text-emerald-600 font-medium">Scheduled</p>
                ...
                <div className="p-2 bg-rose-50 rounded-xl border border-rose-100">
                  <p className="text-[10px] text-rose-600 font-medium">Errors</p>
