...
import { apiPost, getStoredUser, API_BASE_URL, downloadAuthenticatedFile } from '../../lib/api';
...
export default function BulkFeedbackUploadModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/interview-feedback/bulk-upload`, {
        method: 'POST',
        headers: {
  ...
  const pollJobStatus = (jobId, token) => {
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/interview-feedback/bulk-upload/${jobId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const downloadReport = () => {
    ...
    const user = getStoredUser();
    const token = user?.token;
    window.open(`${API_BASE_URL}/interview-feedback/bulk-upload/${jobStatus.jobId}/report?token=${encodeURIComponent(token || '')}`, '_blank');
  };
