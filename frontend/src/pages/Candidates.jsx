...
const Candidates = () => {
  ...
  // Autocomplete and profile retrieval states for Create Candidate modal
  const [selectedCandidate, setSelectedCandidate] = useState(null);
  const [nameSuggestions, setNameSuggestions] = useState([]);
  const [isSearchingNames, setIsSearchingNames] = useState(false);
  const [modalError, setModalError] = useState('');

  // Automatically reset modal state when it opens or closes
  useEffect(() => {
    if (!showCreateModal) {
  ...
  // Debounce and search for candidate names
  useEffect(() => {
    if (!showCreateModal || createForm.fullName.trim().length < 2 || selectedCandidate) {
      setNameSuggestions([]);
      return;
    }
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await apiGet(`/candidates?search=${encodeURIComponent(createForm.fullName.trim())}&limit=8`);
        if (res && res.success) {
  ...
  }, [createForm.fullName, selectedCandidate, showCreateModal]);
  ...
  const hasExistingResume = useMemo(() => {
    return !!(selectedCandidate?.resumeFile?.storageKey || selectedCandidate?.resumeLinkOriginal || selectedCandidate?.resumeFileId);
  }, [selectedCandidate]);
              ...
              <form className="space-y-5" onSubmit={async (e) => {
                ...
                if (selectedCandidate) {
                  ...
                  try {
                    ...
                    // 2. Patch candidate details and status
                    const updatePayload = {
                      ...
                      status: statusFilter && statusFilter !== 'All' ? statusFilter : 'JOINED',
                    };
                    ...
                    setBanner('Candidate profile updated and status set to Joined successfully!');
                    setTimeout(() => setBanner(''), 3000);
                    refetch();
                  } catch (err) {
